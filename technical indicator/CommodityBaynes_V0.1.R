                  ...
                  , ContractType='both' )
{
  ...
  for (i in (1:xtsLength)) {
    if (is.na(Indicator_02[i, 2])==FALSE) {
      ...
      if (Indicator_02[i, 2]>=CCI_Threshold) {
        Indicator_02$SignalFlag[[i]]=1
      } else if (Indicator_02[i, 2]<=CCI_Threshold*-1) {
        Indicator_02$SignalFlag[[i]]=-1
  ...
  for (k in (1:OpportunityCount))
  {
    ...
    # BestPostionCollection=data.frame(BestPostionOut=BestPostionOut, BestPositionOutDate=BestPositionOutDate)
    # BestPosition=rbind(BestPostionCollection, BestPosition)
  ...
  Indicator_04$YearRange=paste0(substr(Indicator_04$Date, 1, 3), 0)
